...
from bdpn.dl import MODEL2TARGET_COLUMNS, BDCT, TRAINING_PATH, BD, BDEI, BDSS
from bdpn.dl import MODEL_PATH, QUANTILES, MODELS, MODEL_FINDER
from bdpn.dl.bdct_model_finder import MODEL_FINDER_PATH
from bdpn.dl.model_serializer import load_model_keras, load_scaler_numpy
...
def predict_parameters(forest_sumstats, model_name=MODEL_FINDER, ci=False):
    ...
    if len(model_ids) == 1:
        ...
        if any('CT' in MODELS[_] for _ in model_ids):
            ...
            bdeiss_ids = {_[0] for _ in enumerate(model_ids) if MODELS[_[1]] in (BD, BDEI, BDSS)}
            for idx in bdeiss_ids:
                results[idx].loc[:, ups_cols] = np.zeros((n_forests, len(ups_cols)), dtype=float)
                results[idx].loc[:, phi_cols] = results[idx].loc[:, psi_cols].to_numpy(dtype=float, na_value=0)
        bdei_ids = {_[0] for _ in enumerate(model_ids) if BDEI in MODELS[_[1]]}
        if bdei_ids and len(bdei_ids) < len(model_ids):
            f_cols = [(f'{F_I}_{q * 100:.1f}' if q != 0.5 else F_I) for q in QUANTILES]
            for idx in range(len(model_ids)):
                if not idx in bdei_ids:
                    results[idx].loc[:, f_cols] = np.zeros((n_forests, len(f_cols)), dtype=float)
        bdss_ids = {_[0] for _ in enumerate(model_ids) if BDSS in MODELS[_[1]]}
        if bdss_ids and len(bdss_ids) < len(model_ids):
            f_cols = [(f'{F_SS}_{q * 100:.1f}' if q != 0.5 else F_SS) for q in QUANTILES]
            x_cols = [(f'{X_SS}_{q * 100:.1f}' if q != 0.5 else X_SS) for q in QUANTILES]
            for idx in range(len(model_ids)):
