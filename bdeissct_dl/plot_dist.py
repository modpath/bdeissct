...
for model in [f'BD{ei}{ss}{ct}' for ei in ['', 'EI'] for ss in ['', 'SS'] for ct in ['', 'CT']]:
    pure_train_data = glob.glob(os.path.join(train_data_prefix, 'train*', '200_500', model, 'trees.train.csv.xz'))
    pure_hist = os.path.join(train_data_prefix, 'pure_models_8', '200_500', f'{model}.hist.pdf')
