...
for n_model in nested_models:
    ...
    for i in range(50):
        ...
        if not os.path.exists(train_nwk) or not os.path.exists(train_log):
            simulate_main(train_nwk, train_log, model_name=n_model, min_tips=mint, max_tips=maxt, n=500,
                          min_R=1, max_R=3, min_d=1, max_d=10, min_rho=0.1, max_rho=0.5,
                          min_d_inc=0.5)

        nwks.append(train_nwk)
        logs.append(train_log)
    train_ss = os.path.join(FOLDER, 'train', f'train_trees.{n_model}.csv.xz')

    if not os.path.exists(train_ss):
        save_forests_as_sumstats(train_ss, nwks=nwks, logs=logs)

    train_data.append(train_ss)
    print('Finished simulating and encoding training data for model', n_model)
...
val_data = []
for n_model in nested_models:
    val_nwk = os.path.join(FOLDER, 'train', f'val_trees.{n_model}.nwk')
    val_log = os.path.join(FOLDER, 'train', f'val_trees.{n_model}.log')
    val_ss = os.path.join(FOLDER, 'train', f'val_trees.{n_model}.csv.xz')
    if not os.path.exists(val_nwk) or not os.path.exists(val_log):
        simulate_main(val_nwk, val_log, model_name=n_model, min_tips=mint, max_tips=maxt, n=500,
                          min_R=1, max_R=3, min_d=1, max_d=10, min_rho=0.1, max_rho=0.5,
                          min_d_inc=0.5)
...
for n_model in nested_models:
    ...
    train_ss = os.path.join(FOLDER, 'train', f'train_trees.{n_model}.chopped.csv.xz')

    if not os.path.exists(train_ss):
        save_forests_as_sumstats(train_ss, nwks=nwks, logs=logs)

    train_data.append(train_ss)
    print('Finished chopping and encoding training data for model', n_model)
...
for model in nested_models:
    ...
    val_ss = os.path.join(FOLDER, 'train', f'val_trees.{n_model}.chopped.csv.xz')
